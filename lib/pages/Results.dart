...
List<EventData> eventsWithFinalResults = [];
List<EventData> eventsWithSearchResults = [];
...
class _ResultsState extends State<Results> with TickerProviderStateMixin {
    ...
    for (var result in allResults) {
      for (var event in allEvents) {
        if (result.eventId == event.id) {
          ...
          eventsWithFinalResults.add(event);
        }
      }
    }

    eventsWithResults = eventsWithResults.toSet().toList();
    eventsWithFinalResults = eventsWithFinalResults.toSet().toList();
    print(eventsWithResults.length);
    print(eventsWithFinalResults.length);
    return eventsWithResults;
  }

  void filterSearchResults(String query) {
    if (eventsWithFinalResults.length == 0) {
      for (var result in allResults) {
        for (var event in allEvents) {
          if (result.eventId == event.id) {
            eventsWithFinalResults.add(event);
    ...
    eventsWithFinalResults.toSet().toList();

    print(eventsWithFinalResults.length);
    List<EventData> searchResults = [];
    for (var event in eventsWithFinalResults) {
    ...
    setState(() {
      ...
      eventsWithResults.clear();
      if (searchResults.length != 0) {
        eventsWithResults = searchResults.toSet().toList();
      } else {
        eventsWithResults = [];
      }
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        floatingActionButton: animatedSearch(context),
          ...
          body: FutureBuilder(
            ...
            builder: (context, snapshot) {
              if (!snapshot.hasData)
                ...
                return eventsWithFinalResults.length != 0
                    ? Container(
  ...
  Widget animatedSearch(BuildContext context) {
